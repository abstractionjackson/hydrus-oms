...
export const actions = {
	create: async ({ locals: { supabase }, request }) => {
		...
		const { data, error } = await supabase
			.from('patient')
			.insert([{ name_last, name_first, dob, case_date, od_os, pre_op_iop, pre_op_meds_count }])
		...
		throw redirect(
			303,
			`/dashboard?redirectFrom=patient/new&id=${data.id}&name_last=${data.name_last}&name_first=${data.name_first}`
		);
	}
} satisfies Actions;
